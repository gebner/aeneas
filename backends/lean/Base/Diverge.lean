...
syntax (name := divergentDef) declModifiers "divergent" "def" declId ppIndent(optDeclSig) declVal : command

open Lean Elab Term Meta Primitives

-- Copied from the `wfRecursion` function.

open WF in
def divRecursion (preDefs : Array PreDefinition) : TermElabM Unit := do
  logInfo <| toMessageData <| preDefs.map fun pd => (pd.declName, pd.levelParams, pd.type, pd.value)

  -- CHANGE HERE This function should add definitions with these names/types/values ^^
  for preDef in preDefs do
    addAsAxiom preDef -- scaffolding
